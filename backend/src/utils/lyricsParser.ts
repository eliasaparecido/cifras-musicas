...
export function isChordOverLyrics(lyrics: string): boolean {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    if (words.length > 0) {
      ...
      if (chordWords.length / words.length > 0.5 && nextLine && nextLine.length > 0) {
        return true;
      }
    }
  }
  ...
  while (i < lines.length) {
    ...
    if (isChordLine && nextLine !== undefined) {
      ...
      for (const word of currentLine.split(/(\s+)/)) {
        if (/^[A-G](#|b)?(m|maj|dim|aug|sus)?(2|4|5|6|7|9|11|13)?$/.test(word.trim())) {
      ...
      for (const { pos, chord } of chordPositions.sort((a, b) => a.pos - b.pos)) {
